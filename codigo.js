...
function listarProductos(productos) {
	...
	var agregarf= document.getElementById("agregarf");
	...
	if (orden === 0) { orden = -1; precio.innerHTML = "Precio" }
		...
		if (orden == 1) { ordenarAsc(productos, "price"); precio.innerHTML = "Precio A"; precio.style.color = "darkgreen" }
			...
			if (orden == -1) { ordenarDesc(productos, "price"); precio.innerHTML = "Precio D"; precio.style.color = "orange" }
	agregarf.style.display = "block";		
...
function obtenerProductos() {
	 fetch('https://retoolapi.dev/ZFECgJ/productos')
		.then(res => res.json())
		.then(data => {
			productos = data;
			productos.forEach(
...
function agregarProductos() {
	...
	var txtTitulo = document.getElementById("NuevoTit").value;
	var txtDescrip = document.getElementById("NuevoDescrip").value;
	var txtPrecio = document.getElementById("Precio").value;
	var txtCat = document.getElementById("NuevoCat").value;
...
function eliminarProductos(id) {
	fetch('https://retoolapi.dev/ZFECgJ/productos/'+id, { method: "DELETE" })
